...
def find_paths(connections: Dict[str, List[str]], end: str = 'end'):
    lowers = [k for k in connections.keys() if k.islower() and k not in ['start', 'end']]
    unfinished_paths = []
    for lower in lowers:
        unfinished_paths.append(Path(['start'], lower))
    finished_paths = []
    while unfinished_paths:
        ...
        for n in connections[path.path[-1]]:
            if can_be_visited(n, path):
                ...
                if n == end:
                    if path.path not in finished_paths:
                        finished_paths.append(path.path)
                else:
                    unfinished_paths.append(new_path)

        print(f"Paths len: {len(finished_paths)}")
        # print(f"Finished Paths: {finished_paths}")
